...
// If the interval j doesn't exist, store -1 for the interval i. Finally, you need output the 
...
// 有些时候想要sort不一定得sort数组，可以用map来构建一个有序的数列
class Solution {
 public:
  vector<int> findRightInterval(vector<Interval>& intervals) {
  	if (intervals.empty())  return vector<int>();
  	vector<int> ans(intervals.size(), -1);
  	// init the map, using end point as key
  	map<int, int> map;
  	for (int i = 0; i < intervals.size(); ++i) {
  		map[intervals[i].start] = i;
  	}
  	// find pair for each interval
  	for (int i = 0; i < intervals.size(); ++i) {
  		auto itr = map.lower_bound(intervals[i].end);
  		if (itr != map.end())  ans[i] = itr->second;
  	}
  	return ans;
  }
};